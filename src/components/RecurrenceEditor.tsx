...
export function RecurrenceEditor({ seedDate, initial, onChange }: Props) {
  ...
  const rule = useMemo<RecurrenceRule | null>(() => {
    ...
    else if (dayType === 'nth_bizday') day = { type: 'nth_bizday', nth: nthBiz }
              ...
              <div className="recur-row recur-params">
                ...
                {dayType === 'nth_bizday' && (
                  <>
                    ...
                    <input
                      ...
                      max={23}
                      value={nthBiz}
                      onChange={(e) => setNthBiz(Number(e.target.value))}
                    />
                    <span>{ordinalSuffix(nthBiz)} business day (from the 1st)</span>
                  </>
                )}
                ...
                {dayType === 'weekday_on_or_after' && (
                  <>
                    ...
                    />
                    <span>{ordinalSuffix(woaDay)}</span>
                    <select value={woaRoll} onChange={(e) => setWoaRoll(e.target.value as 'forward' | 'backward')}>
